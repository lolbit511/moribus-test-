    ...
    def player_hit(self, damage = 10): #player
        if GO.GameEnded == False:
            ...
            if self.cooldown == False: #
                mmanager.playsound(playerHitSound, 0.3)
                ...
                if self.health <= 0:
                    mmanager.playsound(playerDeathSound, 0.3)
                    self.kill()
                    mmanager.stop()
        ...
        if hits:
            ...
            if player.health < 100 and self.type == 1: #############################################################################################################################
                ...
                if player.health > 100:
                    player.health = 100
                mmanager.playsound(healthSound, 0.3)
                #health_ani.image = health_ani[player.health] # old code do not uncomment
                health.image = health_ani[int(player.health / 20)] # error
                self.kill() #############################################################################################################################
            if self.type == 2:
                # handler.money += 1
                player.coin += 1
                ...
                self.kill()
            if self.type == 3:
                mmanager.playsound(boostSound, 0.3)
                print("jump boosted")
...
class Enemy(pygame.sprite.Sprite): #enemy 1  enemy1
    ...
    def update(self): #enemy
        ...
        # Activates upon either of the two expressions being true
        if f_hits or s_hits:
            mmanager.playsound(e1DeathSound, 0.3)
...
class Enemy2(pygame.sprite.Sprite): #second enemy, enemy 2
    ...
    def update(self): # 2nd enemy
        ...
        # Activates upon either of the two expressions being true
        if f_hits or s_hits:
            mmanager.playsound(e2DeathSound, 0.3)
...
class Boss1(pygame.sprite.Sprite): #boss, first boss boss 1
    ...
    def update(self): # boss 1
        ...
        if f_hits or s_hits and self.dmgCD == 25:
            mmanager.playsound(e3HitSound, 0.3)
            #player.health = player.health + 3 # added
            if player.health > 100:
                player.health = 100
            self.cooldown = True
            self.health -= 1
            if self.health < 0:
                mmanager.playsound(e3DeathSound, 0.3)
                self.kill()
                handler.dead_enemy_count += 1
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if int(WIDTH-int(WIDTH*0.2)) <= mouse[0] <= int(WIDTH) and int(HEIGHT-int(HEIGHT*0.1)) <= mouse[1] <= HEIGHT:
                ...
                if player.mana >= 6:
                    ...
                    mmanager.playsound(fsound, 0.3) # sound
